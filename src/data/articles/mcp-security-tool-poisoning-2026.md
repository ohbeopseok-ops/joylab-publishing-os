...
보안팀이 모든 도구 설명을 매번 수동 검토하기는 어렵습니다. 따라서 등록 시점의 도구 목록·스키마·설명을 기준선으로 저장하고, 이후 이름·인자·권한·설명문이 바뀌면 변경 이벤트로 잡는 방식이 실용적입니다. 새 도구가 갑자기 추가되거나 기존 읽기 도구가 쓰기 권한을 요구하기 시작하는 변화는 우선 검토 대상으로 올릴 수 있습니다.

이렇게 하면 Tool Poisoning을 모델의 추론 문제만이 아니라 **구성 변경 탐지 문제**로 바꿀 수 있습니다.
