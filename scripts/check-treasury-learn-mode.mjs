...
assert.match(today, /TODAY'S READING · V0\.3/);
assert.match(today, /핵심 압력/);
assert.match(today, /완충 신호/);
assert.match(today, /오늘 확인/);
assert.match(today, /VALUATION PRESSURE/);
assert.match(today, /TERM PREMIUM/);
assert.match(today, /EPS Revision·외국인 수급은 아직 V0\.3 자동 입력값이 아닙니다/);
...
console.log('Treasury Learn Mode V0.3 contract PASS: glossary + number translations + three-line Today Reading with no fabricated EPS/flow inputs.');
