...
function staticReviewRuntime() {
    ...
    Object.entries(pack.channels).forEach(([channel, cfg]) => {
      cfg.variants.forEach((variant) => {
        ...
        card.innerHTML = '<div class="eyebrow">' + escHtml(channel) + ' · ' + escHtml(variant.id) + '</div>' +
          ...
          '<div class="urlrow"><input type="url" placeholder="실제 게시 URL" value="' + escHtml(variant.publishedUrl || '') + '"' + disabled + '>' +
  ...
  document.getElementById('resetBtn').onclick = () => {
    if (!confirm('이 브라우저에 저장된 검토 상태를 Artifact 원본으로 되돌릴까요?')) return;
    pack = clone(baseline);
...
<div class="wrap">
  ...
  <div class="toolbar"><button id="reviewBtn">REVIEW로 전환</button><button id="approveBtn">APPROVED로 승인</button><button id="downloadPack" class="secondary">업데이트된 Pack 다운로드</button><button id="downloadLog" class="secondary">Publish Log 다운로드</button><button id="resetBtn" class="danger">Artifact 원본으로 초기화</button></div>
