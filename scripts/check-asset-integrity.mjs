...
const imageFiles = walk(publicDir).filter((file) => rasterExts.has(path.extname(file).toLowerCase()));
const failures = [];
let decoded = 0;
...
for (const file of imageFiles) {
  try {
    ...
    failures.push(`${rel(file)}: decode failed: ${error.message}`);
  }
}
...
for (const ref of refs) {
  ...
  if (!fs.existsSync(file)) failures.push(`${ref}: referenced raster asset is missing`);
}

const literatureDir = path.join(publicDir, 'leadership', 'literature');
for (const file of walk(literatureDir).filter((f) => rasterExts.has(path.extname(f).toLowerCase()))) {
  try {
    const meta = await sharp(file).metadata();
    const ratio = meta.width / meta.height;
    const target = 16 / 9;
    if (Math.abs(ratio - target) > 0.01) {
      failures.push(`${rel(file)}: Literature Hero/OG must be 16:9; got ${meta.width}x${meta.height}`);
    }
  } catch {
    // decode failure already reported above
  }
}

console.log(`Asset Integrity Gate: decoded ${decoded} raster assets; checked ${refs.size} source references.`);
