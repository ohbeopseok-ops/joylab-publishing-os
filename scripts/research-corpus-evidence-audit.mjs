...
function fmValue(fm, key) {
  ...
  const importance = Math.min(30,
    (featured ? 8 : 0) +
    (homeFeatured ? 10 : 0) +
    (homePriority !== null && homePriority <= 5 ? 6 : 0) +
    Math.min(6, Math.ceil(claimCount / 5))
  );

  const investmentText = (title + ' ' + category + ' ' + body.slice(0, 3000)).toLowerCase();
  const investmentHits = (investmentText.match(/투자|주가|종목|증시|etf|수익률|밸류에이션|실적|공시|삼성전자|하이닉스|금리|환율|코스피|코스닥/g) || []).length;
  const investmentRisk = Math.min(20, investmentHits * 2);
  ...
  return {
    file, title, category, featured, homeFeatured, homePriority, claims,
    claimCount, mappedCount, weakCount, unmappedCount,
    scoring: {
      priorityScore,
      tier,
      gapScore,
...
const priority = articles
  .filter((x) => x.unmappedCount || x.weakCount)
  .sort((a,b) => b.scoring.priorityScore - a.scoring.priorityScore || b.unmappedCount - a.unmappedCount)
  .slice(0,50);
const top20 = priority.slice(0,20);
const tierCounts = articles.reduce((a,x) => {
  if (x.unmappedCount || x.weakCount) a[x.scoring.tier] = (a[x.scoring.tier] || 0) + 1;
  return a;
}, {P0:0,P1:0,P2:0});
const report = {
  contract:'JoyLab Claim-Source Corpus Audit V1',
  checkedAt:new Date().toISOString(),
  trafficDataStatus:'UNKNOWN — no URL-level current GSC baseline available in repository',
  totals,
  tierCounts,
  top20,
  articles
...
const md = ['# JoyLab Research Claim ↔ Source Corpus Audit V1','', 'Articles: **'+totals.articles+'**', 'Claim candidates: **'+totals.claims+'**', 'Direct: **'+totals.mapped+'**', 'Weak: **'+totals.weak+'**', 'Unmapped: **'+totals.unmapped+'**', '', 'Traffic component: **UNKNOWN / 0 points** until URL-level GSC data is connected.', '', '| Tier | Score | Article | Claims | Direct | Weak | Unmapped | Importance | Investment Risk | Traffic |', '|---|---:|---|---:|---:|---:|---:|---:|---:|---:|', ...top20.map((x) => '| '+x.scoring.tier+' | '+x.scoring.priorityScore+' | '+x.title.replaceAll('|','/')+' | '+x.claimCount+' | '+x.mappedCount+' | '+x.weakCount+' | '+x.unmappedCount+' | '+x.scoring.importance+' | '+x.scoring.investmentRisk+' | '+x.scoring.trafficScore+' |'), '', 'V1 is report-only for the legacy corpus.'].join('\n');
...
if (process.argv.includes('--enforce') && (totals.weak || totals.unmapped)) process.exit(2);, 'm'));
  return m ? m[1] : '';
}

function boolFm(fm, key) {
  return new RegExp('^' + key + ':\\s*true\\s*
  const sourceSection = (body.match(/(?:^|\n)##\s+(?:Sources?|출처|근거)\s*\n([\s\S]*?)(?=\n##\s+|$)/i) || [,''])[1];
  const hasArticleSources = /https?:\/\//.test(sourceSection);
  const paragraphs = body.split(/\n\s*\n/).map((x) => x.trim()).filter(Boolean);
  const claims = [];
...
if (process.argv.includes('--enforce') && (totals.weak || totals.unmapped)) process.exit(2);, 'mi').test(fm);
}
...
function analyze(text, file) {
  ...
  const homePriorityRaw = Number(fmValue(fm, 'homePriority'));
  const homePriority = Number.isFinite(homePriorityRaw) ? homePriorityRaw : null;
  const sourceSection = (body.match(/(?:^|\n)##\s+(?:Sources?|출처|근거)\s*\n([\s\S]*?)(?=\n##\s+|$)/i) || [,''])[1];
  const hasArticleSources = /https?:\/\//.test(sourceSection);
  const paragraphs = body.split(/\n\s*\n/).map((x) => x.trim()).filter(Boolean);
  const claims = [];
